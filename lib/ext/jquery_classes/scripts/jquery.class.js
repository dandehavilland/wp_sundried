...
jQuery.extend(jQuery.Class, {
    ...
    create: function() {
        ...
        function handle() {
            // check if class is _abstracted
            if (this.options._abstract) {
                throw new Error("_abstract classes cannot be instantiated");
            }
        ...
        if (options._implements.length > 0) {
            var attributesMustImplemented = [],
                propertiesMustImplemented = [],
                methodsMustImplemented = [];
            
            // extract elements from interface
            for (var i = 0; i < options._implements.length; i++) {
                jQuery.merge(attributesMustImplemented, options._implements[i].attributes);
                jQuery.merge(propertiesMustImplemented, options._implements[i].properties);
                jQuery.merge(methodsMustImplemented, options._implements[i].methods);
            }
    ...
    parent: function() {
        ...
        if (index == -1 && superClass[superClass.length - 1] != method) {
            superClass[superClass.length - 1].apply(this, args||[args]);
        }
        else {
            superClass[index - 1].apply(this, args||[args]);
        }
    },
		is_ie: function() {
			jQuery.browser
